...
export function TaskForm() {
  const { createTask, setTasks } = useContext(TaskContext);
  ...
  return (
    <form className="header" onSubmit={handleSubmit}>
      <div className="flex justify-between pt-4 transition-all  items-center tablet:pt-2">
        <h1
          className={`text-2xl tracking-[12px] font-bold tablet:text-3xl tablet:tracking-[16px] laptop:text-4xl transition-all`}
        >
          TODO
        </h1>
      </div>
